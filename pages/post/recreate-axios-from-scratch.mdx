...
那么作为调用者，我们希望如果调用这个简化版的 Axios 呢。当然是越简单越好，最好能做到不需要设置任何东西就可以直接发起请求。同时，能提供多种调用方式，以满足不同调用者的使用习惯，例如：

```js
// 默认情况下直接发起 GET 请求
axios("https://www.xxx.com/api");
// 使用 get 方法发起请求
axios.get("https://www.xxx.com/api");
// 指定参数
axios({
  method: "get",
