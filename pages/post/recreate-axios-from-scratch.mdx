...
`mergeConfig()`方法在很多地方有使用到，这些配置的优先级如下：
...
原生的`XMLHttpRequest`使用回调函数来监听各种事件，容易造成回调地狱，也不利于链式调用。Promise 化（Promisify）是常见的面试题，原理很简单，当成功时`resolve()`，当失败/出错时`reject()`。我们把上面原生的 xhr 请求 Promise 化，代码如下：
