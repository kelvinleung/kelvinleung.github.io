...
在网上能找到的资料中，尤其是国内的中文语境下，大多数都是基于旧规范的解析。其核心概念有**AO（Activation Object，活跃对象）**、**VO（Variable Object，变量对象）**、**作用域（Scope Chain）**等。
...
- 在全局执行上下文中，VO 即全局对象`window`（浏览器环境），在初始化的时候，定义了像` Math``String``Date `等属性
